...
/// 渲染比例色条中的单个分段
fn render_bar_segment<State: 'static>(width: f32, color: xilem::Color) -> impl WidgetView<State> {
    sized_box(label(""))
        .width(width.px())
        .height(14.0_f32.px())
...
/// 渲染 Token Proportional Breakdown 组件
pub fn breakdown_view<State: 'static>(data: TokenBreakdownData) -> impl WidgetView<State> {
    // 1. 构建比例色条分段 Vec
    let mut bar_segments = Vec::new();
    if data.w_input > 0.0 {
        bar_segments.push(render_bar_segment(data.w_input, theme::COLOR_INPUT));
    }
    if data.w_output > 0.0 {
        bar_segments.push(render_bar_segment(data.w_output, theme::COLOR_OUTPUT));
    }
    if data.w_cache > 0.0 {
        bar_segments.push(render_bar_segment(data.w_cache, theme::COLOR_CACHE));
    }
    if data.w_reasoning > 0.0 {
        bar_segments.push(render_bar_segment(data.w_reasoning, theme::COLOR_REASONING));
    }
