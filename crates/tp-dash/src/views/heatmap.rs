...
//         // 增量优化：如果总记录数没发生变动，且我们已经计算过，跳过重复计算
//         if summary.record_count == self.last_record_count && !self.data.weeks.is_empty() {
//             return;
//         }
//
...
//
//         // 2. 表演面板包装
//         let panel = crate::views::panel::panel_container(
...
//
//                     // 100% 相对组件顶部的偏移量
//                     // Mon/Wed/Fri 的标签宽度为 50px
//                     let x_offset = 50.0 + (c_idx as f64) * (col_step as f64);
//                     // months_header 高度 = 22.0，与网格间距 8.0
//                     let y_offset = 22.0 + 8.0 + (r_idx as f64) * (col_step as f64);
//
//                     anchor_point = crate::widgets::AnchorPoint::Custom(
//                         x_offset + cell_box_size as f64,
...
//         });
//
//         // 4. 使用 popover_stack 实现完美的悬浮对齐层级表现
//         crate::widgets::popover_stack(
//             panel,
//             hoverable_tooltip,
